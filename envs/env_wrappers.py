...
class SubprocVecEnv(object):
    def __init__(self, all_args):
        ...
        self.env_list = [Env(i, all_args) for i in range(all_args.n_rollout_threads)]
    ...
    def start(self):
        [env.start() for env in self.env_list]
...
class DummyVecEnv(object):
    def __init__(self, all_args):
        ...
        self.env_list = [Env(i, all_args) for i in range(all_args.n_eval_rollout_threads)]
        self.num_envs = all_args.n_eval_rollout_threads

        self.num_agent = self.env_list[0].agent_num
        #NEW: signal_obs_dim and signal_action_dim are used to define the observation and action spaces for the agents.
        self.signal_obs_dim = self.env_list[0].obs_dim
        self.signal_action_dim = self.env_list[0].action_dim
        ...
        self.share_observation_space = []
        share_obs_dim = 0
        for _agent in range(self.num_agent):
            total_action_space = []

            # physical action space
            if self.discrete_action_space:
                #u_action_space = spaces.Discrete(6)  # 6个离散的动作
